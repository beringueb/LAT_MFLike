...
class PlikMFLike(Likelihood):
    ...
    def initialize(self):
        ...
        if not hasattr(self, 'lmax_cut') or self.lmax_cut is None:
            self.lmax_cut = self.lmax_win
    ...
    def load_windows_pliklike(self, weightfile, minfile, maxfile, bin_starts, bin_ends, data_dir = ""):
        ...
        self.idx_keep = np.argwhere(self.b_ell <= self.lmax_cut).T[0]
        self.log.info(f"Keeping {len(self.idx_keep)} bins (out of {len(self.b_ell)})")
        self.row_indices_mesh, self.column_indices_mesh = np.meshgrid(self.idx_keep, self.idx_keep, indexing='ij')
    ...
    def get_requirements(self):
        ...
        } | { k : None for k in self.expected_params }
    
    def get_model(self, cl, fg, **params):
        ...
        ct = 1.0 / np.sqrt(np.array([ params["calib_100T"], 1.0, params["calib_217T"] ]))
        yp = 1.0 / np.sqrt(np.array([ params["calib_100P"], params["calib_143P"], params["calib_217P"] ]))
    ...
    def logp(self, **params_values):
        cl = self.provider.get_Cl(ell_factor = True)
        fg = self.provider.get_Planck_fgs()
        return self.loglike(cl, fg, **params_values)
    
    def loglike(self, cl, fg, **params_values):
        x_model = self.get_model(cl, fg, **params_values)
        diff_vec = self.b_dat[self.idx_keep] - x_model[self.idx_keep]
        
        if self.use_tt and self.use_te and self.use_ee:
            tmp = self.inv_cov[self.row_indices_mesh, self.column_indices_mesh] @ diff_vec
        ...
        raise AttributeError("Should not be using this, ell_cutting not implemented !")
        return -0.5 * np.dot(tmp, diff_vec)
