...
class TheoryForge_MFLike:
    ...
    def get_modified_theory(self, Dls, **params):
        ...
        fg_dict = self._get_foreground_model(**params)
        ...
        cmbfg_dict = self._get_calibrated_spectra(cmbfg_dict, **params)
    ...
    def _init_foreground_model(self):
        ...
        if self._check_component_any_exp("tsz_and_cib", "tt"):
            self.expected_params_fg.append("xi_yc")
    ...
    def _get_foreground_model(self, ell=None, freqs_order=None, **fg_params):
        ...
        print(fg_params)
        if self._check_component_any_exp("tsz_and_cib", "tt"):
            fg_params["a_tszxcib"] = -fg_params["xi_yc"] * np.sqrt(fg_params["a_tSZ"] * fg_params["a_c"])
        if self._check_component_any_exp("tszxcib_spt", "tt"):
            fg_params["a_tszxcib_spt"] = -fg_params["xi_yc"]
        if self._check_component_any_exp("poisson_spt", "tt"):
...
class TheoryForge_PlikMFLike:
    ...
    def get_Planck_foreground(self, fg_params, ell, requested_cls=['tt', 'te', 'ee']):
        ...
        if "tszxcib" in self.fg_component_list["tt"]:
            ...
            szcib_amp[0, 0] = -2.0 * fg_params['xi_yc'] * np.sqrt(
                fg_params['a_tSZ'] * tSZcorr[0] * fg_params['a_c'] * CIBcorr[0])
            szcib_amp[1, 1] = -2.0 * fg_params['xi_yc'] * np.sqrt(
                fg_params['a_tSZ'] * tSZcorr[1] * fg_params['a_c'] * CIBcorr[1])
            szcib_amp[1, 2] = -fg_params['xi_yc'] * np.sqrt(
                fg_params['a_tSZ'] * tSZcorr[1] * fg_params['a_c'] * CIBcorr[2]) - \
                              fg_params['xi_yc'] * np.sqrt(
                fg_params['a_tSZ'] * tSZcorr[2] * fg_params['a_c'] * CIBcorr[1])
            szcib_amp[2, 2] = -2.0 * fg_params['xi_yc'] * np.sqrt(
